...
class TestArchiver(unittest.TestCase):
    ...
    #     layout = np.zeros((20, 8, 8, 2), dtype=np.uint32)
    #     vsource1 = np.ones((80, 8, 2), dtype=np.uint32)
    #     vsource2 = np.ones((80, 8, 2), dtype=np.uint32) * 2
    #     vsources = [vsource1, vsource2]
    #     dataset_index = [0, 0]
    #     num_sources = 2

    #     print(f"layout: {layout.shape}, vsource1: {vsource1.shape} vsource2: {vsource2.shape}")
    #     self.archiver.map_sources_to_layout('pixel_spectra', 0, [20, 20], 2, num_sources, vsources, layout, dataset_index)
    ...
    #     layout = np.zeros((20, 8, 80), dtype=np.uint32)
    #     vsource1 = np.ones((80, 80), dtype=np.uint32)
    #     vsource2 = np.ones((80, 80), dtype=np.uint32) * 2
    #     vsources = [vsource1, vsource2]
    #     dataset_index = [0, 0]
    #     num_sources = 2

    #     self.archiver.map_sources_to_layout('summed_images', 0, [20, 20], 2, num_sources, vsources, layout, dataset_index)

    #     np.testing.assert_array_equal(layout[0:20:2, :, :], vsource1)
    #     self.assertEqual(dataset_index[0], 1)

    # def test_map_sources_to_layout_multiple_datasets_interleaved(self):
    #     """Test map_sources_to_layout with interleaved sources from multiple datasets."""
    #     layout = np.zeros((20, 8, 80), dtype=np.uint32)
    #     vsource1 = np.ones((80, 80), dtype=np.uint32)
    #     vsource2 = np.ones((80, 80), dtype=np.uint32) * 2
    #     vsources = [vsource1, vsource2]
    ...
    #     # Map dataset index 0
    #     self.archiver.map_sources_to_layout('summed_images', 0, [20, 20], num_datasets, num_sources, vsources, layout, dataset_index)

    #     np.testing.assert_array_equal(layout[0:20:2, :, :], vsource1)
    #     self.assertEqual(dataset_index[0], 1)
    #     self.assertEqual(dataset_index[1], 0)

    # def test_map_sources_to_layout_skips_non_matching_indices(self):
    #     """Test map_sources_to_layout skips sources not matching current index."""
    #     layout = np.zeros((20, 8, 80), dtype=np.uint32)
    #     vsource1 = np.ones((80, 80), dtype=np.uint32)
    #     vsource2 = np.ones((80, 80), dtype=np.uint32) * 2
    #     vsources = [vsource1, vsource2]
    ...
    #     # Map dataset index 1 (should skip vsource1 at idx 0, and process vsource2 at idx 1)
    #     self.archiver.map_sources_to_layout('summed_images', 1, [20, 20], num_datasets, num_sources, vsources, layout, dataset_index)

    #     # Layout should remain zero for index 0, and have vsource2 at stride positions
    #     np.testing.assert_array_equal(layout[0, :, :], np.zeros((80, 80)))
    #     np.testing.assert_array_equal(layout[1:20:2, :, :], vsource2)
    #     self.assertEqual(dataset_index[0], 0)
    #     self.assertEqual(dataset_index[1], 1)

    # def test_map_sources_to_layout_empty_vsources(self):
    #     """Test map_sources_to_layout with empty vsources list."""
    #     layout = np.zeros((20, 8, 80), dtype=np.uint32)
    #     vsources = []
    #     dataset_index = [0]
    ...
    #     np.testing.assert_array_equal(layout, np.zeros((20, 8, 80)))
    #     self.assertEqual(dataset_index[0], 0)

    # def test_map_sources_to_layout_stride_behavior(self):
    #     """Test map_sources_to_layout correctly applies stride from num_sources."""
    #     layout = np.zeros((30, 8, 80), dtype=np.uint32)
    #     vsource1 = np.ones((80, 80), dtype=np.uint32)
    #     vsource2 = np.ones((80, 80), dtype=np.uint32) * 2
    #     vsource3 = np.ones((80, 80), dtype=np.uint32) * 3
    #     vsources = [vsource1, vsource2, vsource3]
    #     dataset_index = [0, 0, 0]
    #     num_datasets = 3
    #     num_sources = 3

    #     self.archiver.map_sources_to_layout('summed_images', 0, [30, 30, 30], num_datasets, num_sources, vsources, layout, dataset_index)

    #     np.testing.assert_array_equal(layout[0:30:3, :, :], vsource1)
    #     self.assertEqual(dataset_index[0], 1)

    # def test_map_sources_to_layout_increments_dataset_index(self):
    #     """Test map_sources_to_layout increments dataset_index for each matched source."""
    #     layout = np.zeros((20, 8, 80), dtype=np.uint32)
    #     vsource1 = np.ones((80, 80), dtype=np.uint32)
    #     vsource2 = np.ones((80, 80), dtype=np.uint32) * 2
    #     vsources = [vsource1, vsource2]
    ...
    #     self.assertEqual(dataset_index[0], 6)
    #     self.assertEqual(dataset_index[1], 0)

    # def test_map_sources_to_layout_spectra_bins_all_elements_assigned(self):
    #     """Test map_sources_to_layout assigns all elements for spectra_bins."""
    #     layout = np.zeros((100, 2048), dtype=np.uint32)
    #     vsource = np.arange(100 * 2048).reshape(100, 2048).astype(np.uint32)
    #     vsources = [vsource]
    #     dataset_index = [0]

    #     self.archiver.map_sources_to_layout('spectra_bins', 0, [100], 1, 1, vsources, layout, dataset_index)

    #     np.testing.assert_array_equal(layout, vsource)

    # def test_map_sources_to_layout_pixel_spectra_partial_stride(self):
    #     """Test map_sources_to_layout with pixel_spectra and stride less than num_frames."""
    #     layout = np.zeros((20, 8, 8, 2), dtype=np.uint32)
    #     vsource1 = np.ones((80, 8, 2), dtype=np.uint32)
    #     vsource2 = np.ones((80, 8, 2), dtype=np.uint32) * 2
    #     vsources = [vsource1, vsource2]
    ...
    #     self.archiver.map_sources_to_layout('pixel_spectra', 0, [20, 20], num_datasets, num_sources, vsources, layout, dataset_index)
    #     self.archiver.map_sources_to_layout('pixel_spectra', 1, [20, 20], num_datasets, num_sources, vsources, layout, dataset_index)
