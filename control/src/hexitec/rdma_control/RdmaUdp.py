...
"""Classes and functions to connect, and communicate with `RDMA` control plane via `UDP` Ethernet.

.. important::

   Requires memory mapped :obj:`dict` imported from :mod:`RdmaUdp.UART_RDMA_REGISTERS` and helper
   functions imported from :mod:`RdmaUdp.rdma_register_helpers`.

   :mod:`RdmaUdp.UART_RDMA_REGISTERS` is generated from XML2VHDL output, regenerated at FPGA
   synthesis time. Please ensure the latest version is used in conjunction with
   :mod:`RdmaUdp.RdmaUdp` to ensure compatibility with the register map in the current FPGA
   bitstream.
"""
import socket
import struct
...
class RdmaUDP(object):
    ...
    .. important::
       ...
       Requires memory mapped :obj:`dict` imported from :mod:`RdmaUdp.UART_RDMA_REGISTERS` and
       helper functions imported from :mod:`RdmaUdp.rdma_register_helpers`.
    ...
    .. note::
       ...
       In addition to providing direct access to the FPGA `RDMA` memory-map using
       :meth:`udp_rdma_read` and :meth:`udp_rdma_write`, the FPGA can be used to bridge to
       connected devices and peripherals to access the following:
         - `aSpect` based `UART`

    Args:
        local_ip (:obj:`str`): IP address of the local NIC used to communicate with the remote
            `RDMA` control plane.
        local_port (:obj:`int`, optional): Port used to connect to remote UDP `RDMA` control plane.
            Default: `65535`.
        ...
        udpmtu (:obj:`int`, optional): `Maximum Transfer Unit` for connection to `UDP` `RDMA`
            control plane.
            Default: `9000` (jumbo frame).
        udptimeout (:obj:`int`, optional): Timeout, in seconds, for connection to `UDP` `RDMA`
            control plane.
            Default: `5`
        speed (:obj:`str`, optional): Speed of the Ethernet connection, from: `10G`, `100G`.
            Default: `10G`.
        debug (:obj:`bool`): Flag to enable debug messages. Default: `False`.
        cmd_no (:obj:`int'): Number to include in the constructed `RDMA` header, useful for
            debugging.
    """
    ...
    def __init__(self, local_ip='192.168.0.1', local_port=65535,
        ...
        #     [=] native byte order
        #     [H] unsigned short <Burst Length> | [B] unsigned char <CMD No>
        #     [B] unsigned char <OP Code> | [L] unsigned long <Address>
        ...
        # Width of the readout interface influences the amount of padding required in RDMA
        # read responses
        if self.speed == "100G":
    ...
    def _RdmaResponseChecker(self, msg, address=0, burst_len=1, op_code=0, comment=""):
        ...
        try:
            ...
            rdma_response = ""
            _errorResponse(e, f"{msg} acknowledge failed", address=address,
                           burst_len=burst_len, op_code=op_code, cmd_no=self.cmd_no,
                           comment=comment)
    ...
    def udp_rdma_read(self, address, burst_len=1, comment=''):
        ...
        Issues an `RDMA` formatted read command to the established socket and returns back
        the contents.

        Args:
            address (:obj:`int`): Memory location to read.
            burst_len (:obj:`int`, optional): number of consecutive read transactions, starting
                from :attr:`address`.
                Default: `1`.
            comment (:obj:`str`, optional):Useful for debugging, will be included in debugging
                output.
                Default: `""` (empty string).

        Returns:
            :obj:`list` of :obj:`int`: Data stored in :attr:`address` parsed by
            :meth:`RdmaUDP_RdmaResponseChecker`.
        """
        ...
        return self._RdmaResponseChecker("Read", address=address,
                                         burst_len=burst_len, op_code=op_code, comment=comment)
    ...
    def udp_rdma_write(self, address, data, burst_len=1, comment=''):
        ...
        Args:
            ...
            data (:obj:`list` of :obj:`int`): data to write, starting at :attr:`address`. *Note*,
                the number items in this :obj:`list` must be greater or equal to the requested
                :attr:`burst_len`. :attr:`burst_len` is used to determine the number of
                consecutive writes to perform, not the number of items in this :obj:`list`.
            burst_len (:obj:`int`, optional): number of consecutive write transactions, starting
                from :attr:`address`.
                Default: `1`.
            comment (:obj:`str`, optional):Useful for deugging, will be included in debugging
                output.
                Default: `""` (empty string).

        Whilst nothing is returned the response from the write command is parsed by
        :meth:`RdmaUDP_RdmaResponseChecker`.
        ...
        if self.debug:
            _debugMsg("RDMA Write", address=address,
                      burst_len=burst_len, op_code=op_code, cmd_no=cmd_no,
        ...
        command_0 = struct.pack(packet_struct_str, burst_len, cmd_no_0, op_code,
                                address, *data_as_list)
        if self.debug:
            print("[DEBUG]: RDMA Write request:")
        ...
        write_reponse = self._RdmaResponseChecker("Write", address=address,
                                                  burst_len=burst_len, op_code=op_code,
                                                  comment=comment)
    ...
    def uart_write(self, wr_d):
        ...
        Args:
            wr_d (:obj:`list` of bytes as :obj:`int`): Data to write to the `UART`. When the entire
                :obj:`list` has been transferred to the `UART` write buffer, the complete buffer
                will be transmitted.
    ...
    #         rx_fifo_reg = self.udp_rdma_read(rx_fifo_dict['addr'] + self.uart_offset, burst_len=1,
    ...
    #
    #     self.udp_rdma_write(tx_fifo_dict['addr'] + self.uart_offset, [dev_addr], burst_len=1,
    #                         comment=tx_fifo_dict['description'])
    #     # Enable Transmission
    #     cr_reg = self.udp_rdma_read(cr_dict['addr'] + self.uart_offset, burst_len=1,
    ...
    #     self.udp_rdma_write(cr_dict['addr'] + self.uart_offset, [cr_reg], burst_len=1,
    #                         comment=cr_dict['description'])
    #
    #     if not self._poll_iic_isr_flag("INT_2", idx=idx, operation="write device addr",
    #                                    dev_addr=dev_addr,  attempts=1000): return 0
    #
    ...
    #     self.udp_rdma_write(cr_dict['addr'] + self.uart_offset, [cr_reg], burst_len=1,
    #                         comment=cr_dict['description'])
    #     self.udp_rdma_write(tx_fifo_dict['addr'] + self.uart_offset, [b], burst_len=1,
    #                         comment=tx_fifo_dict['description'])
    #
    #     if not self._poll_iic_isr_flag("INT_2", idx=idx, operation="write byte",
    #                                    dev_addr=dev_addr,  attempts=1000): return 0
    #     # clear Tx FIFO and completed interrupts:
    #     isr_reg = self.udp_rdma_read(isr_dict['addr'] + self.uart_offset, burst_len=1,
    ...
    #     self.udp_rdma_write(isr_dict['addr'] + self.uart_offset, [isr_reg], burst_len=1,
    ...
    #     SOFTR_KEY = 0xA
    #     self.udp_rdma_write(softr_dict['addr'], [SOFTR_KEY], burst_len=1,
    ...
    #     cr_reg = self.udp_rdma_read(cr_dict['addr'], burst_len=1,
    #                                 comment=cr_dict['description'])[0]
    #     cr_reg = set_field(cr_dict, "EN", cr_reg, 1)
    #     self.udp_rdma_write(cr_dict['addr'], [cr_reg], burst_len=1,
    #                         comment=cr_dict['description'])
    #
    #     isr = self.udp_rdma_read(isr_dict['addr'], burst_len=1,
    #                              comment=isr_dict['description'])[0]
    #     isr_clr = isr & ISR_CLR_MASK
    #     self.udp_rdma_write(isr_dict['addr'], [isr_clr], burst_len=1,
    #                         comment=isr_dict['description'])
    #     isr = self.udp_rdma_read(isr_dict['addr'], burst_len=1,
    #                              comment=isr_dict['description'])[0]
    #     if isr != ISR_INIT_VAL:
    ...
    #         return 0
    #     sr = self.udp_rdma_read(sr_dict['addr'], burst_len=1,
    ...
    #     self.udp_rdma_write(rx_fifo_pirq_dict['addr'], [size - 2], burst_len=1,
    #                         comment=rx_fifo_pirq_dict['description'])
    #     self.udp_rdma_write(tx_fifo_dict['addr'], [(dev_addr << 1) | 0x1], burst_len=1,
    #                         comment=tx_fifo_dict['description'])
    #     # 2 CR MSMS = 1 CR Tx = 0
    #     cr_reg = self.udp_rdma_read(cr_dict['addr'], burst_len=1,
    #                                 comment=cr_dict['description'])[0]
    ...
    #     self.udp_rdma_write(rx_fifo_pirq_dict['addr'], 0, burst_len=1,
    #                         comment=rx_fifo_pirq_dict['description'])
    #     rd_data.extend(self._iic_read_fifo(1, idx=idx, cmd_no=cmd_no))
    #
    #     isr_reg = self.udp_rdma_read(isr_dict['addr'], burst_len=1,
    #                                  comment=isr_dict['description'])[0]
    #     isr_reg = set_field(isr_dict, "INT_3", isr_reg, 1)
    #     self.udp_rdma_write(isr_dict['addr'], [isr_reg], burst_len=1,
    #                         comment=isr_dict['description'])
    #     # 12
