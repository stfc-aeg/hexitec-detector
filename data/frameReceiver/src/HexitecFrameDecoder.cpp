/*
 * HexitecFrameDecoder.cpp
 *
 * ODIN data frame decoder plugin for HEXITEC detector UDP frame data.
 *
...
//  }
  ...
  packets_ignored_ = 0;
  packets_lost_ = 0 ;
  for (int fem = 0; fem < Hexitec::max_num_fems; fem++)
  {
    fem_packets_lost_[fem] = 0;
  }
}

void HexitecFrameDecoder::request_configuration(const std::string param_prefix,
...
//! HEXITEC frame header.
//!
//! \return size of the frame header in bytes
const size_t HexitecFrameDecoder::get_frame_header_size(void) const
{
  return sizeof(Hexitec::FrameHeader);
}
...
//! is the size of the HEXITEC packet header.
//!
...
//!
void* HexitecFrameDecoder::get_packet_header_buffer(void)
{
...
//!
void HexitecFrameDecoder::initialise_frame_header(Hexitec::FrameHeader* header_ptr)
{
  ...
  memset(header_ptr->fem_rx_state, 0,
      sizeof(Hexitec::FemReceiveState) * Hexitec::max_num_fems);
...
//!
size_t HexitecFrameDecoder::get_next_payload_size(void) const
{
...
//!
void HexitecFrameDecoder::monitor_buffers(void)
{
  ...
  while (buffer_map_iter != frame_buffer_map_.end())
  {
    ...
    Hexitec::FrameHeader* frame_header = reinterpret_cast<Hexitec::FrameHeader*>(buffer_addr);
...
//!
void HexitecFrameDecoder::get_status(const std::string param_prefix,
...
//!
uint32_t HexitecFrameDecoder::get_subframe_counter(void) const
{
  return reinterpret_cast<Hexitec::PacketHeader*>(current_packet_header_.get())->subframe_counter;
}
...
//!
unsigned int HexitecFrameDecoder::elapsed_ms(struct timespec& start, struct timespec& end)
{
...
std::size_t HexitecFrameDecoder::parse_fem_port_map(const std::string fem_port_map_str)
{
    ...
    {
        if (buf_idx >= Hexitec::max_num_fems) {
